...
function combo($selected, $default, $idLoc) {
	try {
		$base = new PDOConfig ();
		
		$sql = "SELECT idCentroDistribucion,Nombre,idLocalidad
				FROM centrodistribucion where idLocalidad = $idLoc
				ORDER BY Nombre ";
		//echo $sql;
		$res = $base->query ( $sql );
		...
		foreach ( $res as $row ) {
			if ($selected == $row ["idCentroDistribucion"]) {
				$Listcombo .= "<option value='" . $row ["idCentroDistribucion"] . "' selected='selected'>" . $row ["Nombre"] . "</option>";
			} else {
				$Listcombo .= "<option value='" . $row ["idCentroDistribucion"] . "' >" . $row ["Nombre"] . "</option>";
			}
		}
		echo $Listcombo;
	} catch ( Exception $e ) {
...
function table() {
	try {
		...
		$sql = "SELECT C.idCentroDistribucion,C.Nombre,C.idLocalidad,L.Descripcion AS localidad
				FROM centrodistribucion C
			...
			foreach ( $res as $row ) {
				$tabla .= "<tr>
							...
							<td class='tac'>
								...
								<li><a class='btn tip' title='Modificar' href='#' data-original-title='Edit entry' onclick=\"editar('". $row ["idCentroDistribucion"] ."','".$row ["Nombre"]."','". $row ["idLocalidad"] ."')\">
...
function add($descripcion,$idLocalidad) {
	try {
		...
		$sql = "INSERT INTO centrodistribucion(idLocalidad,Nombre) VALUES ($idLocalidad,'$descripcion')";
		// echo $sql;
...
function update($id,$idLoc,$descripcion) {
	try {
		$base = new PDOConfig ();
		...
		$sql = "UPDATE centrodistribucion
		SET Nombre = '$descripcion',idLocalidad = $idLoc
		WHERE idCentroDistribucion =".$id;
	...
	try {
		...
		switch ($oper) {
			case 'combo' :
				...
				$rta = combo( $selected, $default,$idLocalidad);
				break;	
			case 'add' :
